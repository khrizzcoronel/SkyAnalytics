...
### Ingesta de Datos y ETL (CU-O03, CU-O04, CU-O21, CU-O22)
- `[ ]` **CA-F01:** Verificar que el Cron Job en GitHub Actions (`etl-flights.yml`) arranca correctamente a las 03:00 AM.
- `[ ]` **CA-F02:** Verificar que el script Python `etl_flights_to_monetdb.py` extrae correctamente los datos de `PocketBase.flights_raw`, genera el esquema en estrella (dims + fact_flights) en MonetDB y refresca las vistas analíticas.
...
- `[ ]` **CA-F04:** Verificar que el entrenamiento de Machine Learning (CU-O20) levanta una Instancia EC2 Spot, ejecuta el entrenamiento XGBoost, guarda el artefacto `.pkl` en el Bucket S3, y **se apaga automáticamente** al finalizar (Auto-Shutdown). *(PENDIENTE DE IMPLEMENTACIÓN)*
