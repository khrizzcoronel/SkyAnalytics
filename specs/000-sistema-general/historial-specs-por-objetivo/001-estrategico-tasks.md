...
*   **Descripción:** Ejecutar scripts SQL para crear las vistas `vw_bsc_monthly` y `vw_delay_analysis` basadas en la tabla de hechos `fact_flights` cargada por el pipeline ETL.
*   **Criterio de Completado:** Las vistas existen en MonetDB y devuelven agregados reales de vuelos una vez que el ETL finaliza.
