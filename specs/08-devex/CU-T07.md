...
*   **RN-T07-002 (Autenticación del Sandbox):** Las llamadas al Sandbox no requieren un API Key corporativa real, pero sí un token temporal generado en el navegador para evitar abusos (rate limit de 10 req/minuto estricto).
...
*   **Auto-publicación:** La documentación se publica automáticamente en cada merge a `main`; no hay paso manual de "subir a CDN".
*   **Sandbox aislado:** El sandbox de la API usa datos de prueba sintéticos y nunca toca producción ni datos reales de tenants.
