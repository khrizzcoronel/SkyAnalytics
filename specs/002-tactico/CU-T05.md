...
## 2. Objetivo
Permitir a los Desarrollador (Tú)s diseñar, ejecutar experimentos, entrenar y versionar modelos de Machine Learning (ej. predicción de retrasos, clima) en un entorno estructurado (MLOps) garantizando trazabilidad total de hiperparámetros y métricas.

## 3. Actores Involucrados
*   **Actor Principal:** Desarrollador (Tú)
*   **Sistemas Externos / Actores Secundarios:** Tracking básico o local (Tracking y Model Registry), Archivos Parquet locales, Entorno local o Google Colab o clúster de cómputo PaaS.
...
*   **RF-T05-001:** El sistema (vía SDK interno) debe registrar automáticamente en **Tracking básico o local** cada corrida (Run) de experimentación, guardando: código, hiperparámetros, métricas (MAPE, RMSE) y artefactos (el modelo serializado en formato `.pkl` u ONNX).
*   **RF-T05-002:** El sistema debe consumir las características (Features) unificadas y pre-calculadas directamente desde **Almacenamiento local** para garantizar que los datos usados en entrenamiento sean idénticos a los usados en inferencia (evitando *train-serving skew*).
*   **RF-T05-003:** El sistema debe proveer una interfaz visual (Tracking básico o local UI) para comparar métricas entre diferentes ejecuciones y modelos en paralelo.
...
*   Datos de entrenamiento: Batch extraído históricamente desde Almacenamiento local Feature Store.

## 9. Salidas
*   **Archivos:** Modelo entrenado (formato ONNX o Pickle), Gráficos SHAP (PNG/PDF).
*   **Metadatos:** Registros en la base de datos de Tracking básico o local de las métricas obtenidas.
...
**Entonces** el script envía las métricas (MAPE=12.5%) a Tracking básico o local
**Y** sube el archivo de modelo como artefacto a S3 gestionado por Tracking básico o local
...
*   **CA-T05-001:** Cualquier modelo entrenado puede ser reproducido de manera idéntica ejecutando el mismo commit de código con el mismo Dataset ID proveniente de Almacenamiento local.
