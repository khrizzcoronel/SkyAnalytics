...
**Gestionar Infraestructura con Scripts básicos de despliegue (IaC)**

## 2. Objetivo
Centralizar, versionar y automatizar el aprovisionamiento de toda la infraestructura cloud (AWS, PaaS, CDN, BDs) mediante Infrastructure as Code (Scripts básicos de despliegue), eliminando cambios manuales (ClickOps) y garantizando la reproducibilidad de entornos.
...
*   **RF-T03-002:** El estado de Scripts básicos de despliegue (`Scripts básicos de despliegue.tfstate`) debe almacenarse de forma remota, segura (S3) y bloqueada para prevenir condiciones de carrera (DynamoDB lock).
*   **RF-T03-003:** El sistema de CI/CD debe ejecutar automáticamente `Scripts básicos de despliegue plan` al crear un Pull Request y postear el plan de ejecución como comentario en GitHub.
*   **RF-T03-004:** El sistema de CI/CD debe ejecutar `Scripts básicos de despliegue apply` únicamente cuando el Pull Request es aprobado por al menos otro SRE y fusionado a la rama `main`.
...
*   **RNF-T03-001 (Trazabilidad):** Cada recurso creado debe incluir etiquetas (Tags) obligatorias: `Environment`, `Project`, `Owner`, `ManagedBy=Scripts básicos de despliegue`.
...
*   **RN-T03-002 (Destrucción Protegida):** Recursos con estado en producción (Bases de datos, KMS Keys) deben tener activado `prevent_destroy = true` a nivel de Scripts básicos de despliegue.

## 8. Entradas
*   Código Scripts básicos de despliegue (`.tf` files) modificado en un Pull Request.
*   Variables de entorno y secretos inyectados por GitHub Secrets / .env secrets durante el pipeline.

## 9. Salidas
*   **Log de Ejecución:** Salida en consola de `Scripts básicos de despliegue apply`.
...
**Cuando** sube los cambios en Scripts básicos de despliegue y crea un Pull Request
**Entonces** GitHub Actions ejecuta `Scripts básicos de despliegue plan` y escanea con `tfsec`
**Y** al no encontrar violaciones de seguridad, el plan se adjunta al PR
**Cuando** el Desarrollador (Tú) aprueba y fusiona el PR
**Entonces** el pipeline ejecuta automáticamente `Scripts básicos de despliegue apply` aprovisionando la réplica en AWS.
...
*   **CA-T03-002:** Si el pipeline de Scripts básicos de despliegue falla a la mitad de un apply, el mecanismo de lock en DynamoDB previene ejecuciones simultáneas corruptas.

## 12. Restricciones
*   El pipeline de GitHub Actions debe asumir un rol (OIDC - OpenID Connect) Logsral en AWS, quedando estrictamente prohibido el uso de Access Keys permanentes a nivel de repositorio.

## 13. Fuera de Alcance
*   Aprovisionamiento y configuración interna del software de los contenedores (Scripts básicos de despliegue gestiona la infraestructura base (PaaS/EKS), pero los servicios se despliegan vía Helm o ArgoCD).
