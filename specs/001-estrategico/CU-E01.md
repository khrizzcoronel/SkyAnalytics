...
*   **RF-E01-002:** El sistema debe cargar los indicadores clave (KPIs) desde las tablas `Fact_Suscripcion` (Financiero), `Fact_API_Call` (Procesos), `Fact_Satisfaccion` (Cliente) y `Fact_Incidente` (Procesos) consolidadas en el Data Warehouse.
...
*   **Payload JSON (GraphQL / REST):**
    ...
    *   Serie Logsral (en caso de drill-down): `[ { fecha, valor } ]`
*   **UI:** Interfaz gráfica renderizada en React/Next.js con widgets de KPIs y gráficos de serie Logsral.
...
**Entonces** el sistema solicita la serie Logsral de los últimos 12 meses
...
*   La UI no debe almacenar Logsralmente (cachear en navegador local) información financiera sin encriptar.
