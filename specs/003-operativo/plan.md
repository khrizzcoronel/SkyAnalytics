...
*   **`Fact_Suscripcion` / `Fact_Billing`**: Ingresos de clientes.
*   **`Fact_API_Call`**: Telemetría de uso del sistema.
*   **`Quarantine_Data`**: Nueva tabla arquitectónica (según clarificación global) donde el ETL enviará las filas corruptas, mal formadas o sin valores críticos para su revisión posterior, evitando frenar la carga del lote sano.
