...
*   **RF-O05-002:** El sistema debe invocar a Almacenamiento local para generar un dataset de entrenamiento combinando datos estáticos de aeropuertos, históricos de vuelos y condiciones climáticas del último semestre (Ventanilla Logsral).
...
*   **RNF-O05-002:** Todo el output y logs de entrenamiento deben guardarse asíncronamente en Tracking básico o local sin pérdida de datos en caso de que la instancia se apague sorpresivamente (Spot preemption).

## 7. Reglas de Negocio
*   **RN-O05-001 (Promoción Automática Champion-Challenger):** Si el Challenger mejora la precisión (MAPE) del Champion por un margen superior al 2% absoluto, el sistema etiqueta al nuevo modelo como `Staging-Candidate` en Tracking básico o local. Nunca despliega directo a producción sin supervisión.
*   **RN-O05-002 (Criterio de Degradación):** Si el reentrenamiento genera un modelo peor que el histórico (cosa que ocurre cuando hay datos ruidosos), el pipeline se cancela silenciosamente y se mantiene el modelo Champion operativo.

## 8. Entradas
*   Código de entrenamiento (Python scripts desde main branch).
*   Dataset extraído desde Almacenamiento local Store (Múltiples TB de datos en Parquet).

## 9. Salidas
*   **Artefacto:** Archivo modelo (.pkl / ONNX).
*   **Metadata:** Ejecución registrada en Tracking básico o local.
...
**Y** el pipeline marca al nuevo modelo en Tracking básico o local como `Staging-Candidate`
...
**Y** el sistema archiva (Archived) la corrida en Tracking básico o local
**Y** detiene el pipeline sin emitir alertas críticas, manteniendo el sistema productivo estable.

## 11. Criterios de Aceptación
*   **CA-O05-001:** El proceso de extracción del dataset (Almacenamiento local) para el reentrenamiento debe generar datos que incluyan variables con un rezago máximo de 48 horas respecto a la fecha de ejecución (Frescura de Features).
*   **CA-O05-002:** Se genera un reporte de explicabilidad (SHAP Values) adjunto en Tracking básico o local para auditar por qué el nuevo modelo tomó ciertas decisiones en el test set.
