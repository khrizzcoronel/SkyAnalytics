...
## 2. Objetivo
Permitir al Desarrollador (Tú) y a Auditores Externos evaluar y verificar el estado de cumplimiento de las normativas de seguridad, privacidad y operatividad (SOC 2, ISO 27001, GDPR, y regulaciones IATA) mediante un diagnóstico visual del avance de controles.
...
SkyAnalytics procesa datos sensibles y requiere certificaciones robustas (SOC 2 / ISO 27001) para vender licencias a clientes Enterprise y gubernamentales. Llevar un control manual de las evidencias técnicas es propenso a errores y omisiones, por lo que se requiere un panel automatizado de compliance.
...
*   **RN-E04-002 (Cifrado Mandatorio):** El control de cifrado en reposo y en tránsito no admite excepciones (Zero Trust). Si AWS Security Hub detecta un recurso no cifrado, el control SOC 2 falla automáticamente.
...
**Dado** que el Desarrollador (Tú) visualiza el marco "SOC 2"
...
## 13. Fuera de Alcance
*   Autocorrección de vulnerabilidades de infraestructura (Las brechas de SOC 2 deben solucionarse modificando el código Terraform, no mediante botones en este dashboard).

## 14. Aclaraciones Globales (Speckit-Clarify)
*   **Inmutabilidad de evidencias:** Object Lock debe configurarse en el bucket de evidencias antes de subir el primer PDF; no se acepta "inmutabilidad simulada" por políticas de bucket.
*   **Firma digital:** La clave privada para firmar reportes reside en AWS KMS o equivalente; nunca en el repositorio.
